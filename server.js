...
app.get('/progress.png', async (req, res) => {
    try {
        ...
        if (percent > 0) {
            ...
            gradient.addColorStop(0, '#0f9dde');
            gradient.addColorStop(1, '#0b7ab8');

            ctx.fillStyle = gradient;
            ctx.beginPath();
            // Clip the progress bar to the rounded container shape to ensure left corners are rounded
            // and simply draw the rect.
            // Actually, simpler: draw rounded rect for the bar.
            // Note: roundRect support in node-canvas might vary, using arcTo is safer or just standard roundRect if available in recent versions.
            // node-canvas 2.11+ supports roundRect.
            
            // To handle the right side being flat if < 100%? 
            // Usually progress bars are rounded on the left and flat on right unless 100%.
            // But pill shape usually implies rounded ends for the bar itself too.
            // Let's just draw a rounded rect for the filled part.
